...
# ===================================== Data/Model =====================================
# geo3k: pure image (no video), single image per sample — used to isolate whether
# the Mooncake + Qwen3-VL `reorganize_inputs` assertion is onethinker-specific
# (mixed image/video, multi-media-per-sample) or generic to Mooncake-VL path.
# See ws/tq_breakdown_test/BUG_mooncake_vl_assertion.md for hypothesis.
train_files=/data/verl/geo3k/train.parquet
test_files=/data/verl/geo3k/test.parquet
...
# Bring up a local mooncake_master so this run uses MooncakeStore over RDMA with GDR.
# Mirrors ws/long_run_test/acc_text_7b_tq_gdr.sh — see that file for design notes.
...
# USE_GDR=1: TQ-GDR variant (this file's namesake).  USE_GDR=0: same script can
# be reused to produce the TQ+Mooncake-without-GDR baseline so perf/accuracy can
# be compared against the GDR variant under identical Mooncake backend.
USE_GDR=${USE_GDR:-1}
if [[ "$USE_GDR" == "1" ]]; then
    USE_GDR_CFG="true"
else
...
TQ_BACKEND_CONFIG="
    ...
    +transfer_queue.backend.MooncakeStore.use_gdr=${USE_GDR_CFG}"

# wandb — experiment name auto-suffixed by GDR mode so the two variants share
# the same script but write to different checkpoint dirs.
project_name=${PROJECT_NAME:-pinjiex_megatron_vllm_2node_geo3k_30b_vl_0519}
if [[ "$USE_GDR" == "1" ]]; then
    experiment_name=${EXPERIMENT_NAME:-grpo-tq-gdr-geo3k}
else
    experiment_name=${EXPERIMENT_NAME:-grpo-tq-mooncake-geo3k}
fi
