...
# 24h long-run stability test: VERL + TransferQueue + Mooncake.
#
# Differences from ws/tq_mooncake_datapath_refactor_test/acc_text_30b_vl_onethinker_tq_mooncake.sh:
#   - Single-node: ARNOLD_WORKER_NUM=1 (this run only has 1 H200-NVL node / 8 GPUs)
#   - Dataset swapped from onethinker → geometry3k (model stays Qwen3-VL-30B-A3B-Instruct)
#   - Checkpoint saving DISABLED: save_freq=-1 (disk too small to hold ckpts)
#   - High total_training_steps so the 24h SLURM job is the limiting factor
#   - experiment_name disambiguated for separate wandb / ckpt namespace
...
actor_model_path=/data/verl/Qwen3-VL-30B-A3B-Instruct
...
MOONCAKE_SEGMENT_SIZE=${MOONCAKE_SEGMENT_SIZE:-8589934592}
MOONCAKE_BUFFER_SIZE=${MOONCAKE_BUFFER_SIZE:-2147483648}
LOG_ROOT=${LOG_ROOT:-$PWD/ws/long_run_test_24h/run_logs}
mkdir -p "$LOG_ROOT"
MOONCAKE_MASTER_LOG=$LOG_ROOT/mooncake_master_$(date +%Y%m%d_%H%M%S).log
...
TQ_BACKEND_CONFIG="
    ...
    +transfer_queue.backend.MooncakeStore.use_gdr=false"

# ===================================== Checkpoint / Long-run =====================================
# Weight saving is DISABLED for this run: disk is too small to hold a ckpt
# (~408 GB each, ~816 GB transient while writing) under the 1 TB quota.
# save_freq=-1 makes verl skip _save_checkpoint() entirely (see ray_trainer.py:
# saving only runs when save_freq > 0). With no checkpoints on disk,
# resume_mode=auto simply starts fresh, and max_*_ckpt_to_keep are no-ops.
SAVE_FREQ=${SAVE_FREQ:--1}
MAX_CKPT_KEEP=${MAX_CKPT_KEEP:-1}
TOTAL_TRAINING_STEPS=${TOTAL_TRAINING_STEPS:-2000}
TOTAL_EPOCHS=${TOTAL_EPOCHS:-15}

project_name=${PROJECT_NAME:-pinjiex_longrun_24h_30b_vl_tq_mooncake}
experiment_name=${EXPERIMENT_NAME:-grpo-tq-mooncake-geo3k-30b-vl-longrun-24h}
...
if ls "$default_local_dir"/global_step_* 1>/dev/null 2>&1; then
    echo "Checkpoint found in $default_local_dir, will resume via trainer.resume_mode=auto"
else
