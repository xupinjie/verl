...
if [[ "$USE_MOONCAKE" == "1" ]]; then
    ...
    # GDR enabled — this is the "with GDR" variant. The MR-cache path is exercised on
    # every CUDA-tensor PUT/GET. Compare run time and metrics against acc_text_7b_tq.sh
    # (same script without use_gdr=true).
    TQ_BACKEND_CONFIG="
        ...
        +transfer_queue.backend.MooncakeStore.use_gdr=true"
else
    echo "ERROR: GDR test requires MooncakeStore (USE_MOONCAKE=1). Aborting." >&2
    exit 1
fi

# wandb
project_name=${PROJECT_NAME:-pinjiex_${TASK_NAME}_longrun_0508}
experiment_name=${EXPERIMENT_NAME:-qwen3-8B-$adv_estimator-$backend-$rollout_name-tq-gdr}
