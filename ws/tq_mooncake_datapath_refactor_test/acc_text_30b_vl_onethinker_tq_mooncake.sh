...
# Smoke test for the MooncakeStoreClient data-path refactor.
# Differences from ws/tq_breakdown_test/acc_text_30b_vl_onethinker_tq_gdr.sh:
#   - TOTAL_TRAINING_STEPS defaults to 3
#   - USE_GDR fixed to false: the refactored path packs values into a CPU
#     uint8 buffer, so the GDR flag is a no-op for this run
#   - No VERL_TQ_MC_GET_RETRIES / VERL_TQ_MC_GET_RETRY_DELAY_S workaround;
#     the refactor removes the old bytes-path that needed it
#   - experiment_name + log dir disambiguated from the breakdown_test runs
...
# ===================================== Data/Model =====================================
train_files=/data/verl/onethinker_parquet/train.parquet
test_files=/data/verl/onethinker_parquet/val.parquet
...
MOONCAKE_MASTER_LOG=/tmp/mooncake_master_$(date +%Y%m%d_%H%M%S).log
...
# The refactored MooncakeStoreClient packs every value into a CPU uint8 buffer
# before handing pointers to batch_upsert_from / batch_get_into. There is no
# GPU-side registered buffer in the new path, so use_gdr is fixed to false.
TQ_BACKEND_CONFIG="
...
project_name=${PROJECT_NAME:-pinjiex_megatron_vllm_2node_onethinker_30b_vl_0324}
experiment_name=${EXPERIMENT_NAME:-grpo-tq-mooncake-datapath-refactor}
...
python3 -m verl.trainer.main_ppo_sync \
    ...
    $ROLLOUT_CONFIG \
    $TQ_BACKEND_CONFIG \
    trainer.total_training_steps=${TOTAL_TRAINING_STEPS:-3} \
